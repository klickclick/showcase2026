...
const TeamGrid: React.FC<TeamGridProps> = ({ teams, onSelectTeam }) => {
  return (
    <motion.div
      ...
      className="min-h-screen w-full flex flex-col px-4 md:px-8 py-24 md:py-32"
    >
      <motion.h1
          ...
          >
            ...
            <div className="absolute inset-0">
              <img
                src={team.image}
                alt={team.name}
                className={`w-full h-full transition-all duration-700 ease-out group-hover:scale-110 ${team.imageStyle || 'object-cover'} opacity-100 md:opacity-100 md:group-hover:opacity-100`}
              />
            ...
            <div className="absolute inset-0 flex flex-row md:flex-col items-center md:items-start justify-between md:justify-end p-6 md:p-8">
              <div className="flex flex-col z-10 w-full">
                <h2 className={`${team.name.length > 25 ? 'text-2xl md:text-3xl' : 'text-3xl md:text-5xl lg:text-6xl'} font-display font-bold uppercase md:tracking-wide leading-tight text-white drop-shadow-lg`}>
                  {team.name}
                </h2>
